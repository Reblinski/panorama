...
const xe = class z {
  ...
  execute(e, t) {
    ...
    for (let f = 0; f < a.length; f++) {
      ...
      const U = ((i = n.globalUniforms.globalUniformData) == null ? void 0 : i.worldColor) ?? 4294967295, b = p.color, P = (U >>> 24 & 255) / 255, X = (b >>> 24 & 255) / 255, se = ((c = n.filter) == null ? void 0 : c.alphaMultiplier) ?? 1, te = P * X * se;
      if (te <= 0) continue;
      r.globalAlpha = te;
      const ne = U & 16777215, J = b & 16777215, w = Re(ce(J, ne)), S = y.frame, I = v.addressModeU ?? v.addressMode, de = v.addressModeV ?? v.addressMode, ae = z._getPatternRepeat(I, de), q = y.source._resolution ?? y.source.resolution ?? 1, $ = (l = (h = p.renderable) == null ? void 0 : h.renderGroup) == null ? void 0 : l.isCachedAsTexture, m = S.x * q, C = S.y * q, _ = S.width * q, L = S.height * q, A = p.bounds, k = n.renderTarget.renderTarget.isRoot, G = A.minX, O = A.minY, B = A.maxX - A.minX, W = A.maxY - A.minY, N = y.rotate, u = y.uvs, Y = Math.min(u.x0, u.x1, u.x2, u.x3, u.y0, u.y1, u.y2, u.y3), j = Math.max(u.x0, u.x1, u.x2, u.x3, u.y0, u.y1, u.y2, u.y3), M = ae !== "no-repeat" && (Y < 0 || j > 1), pe = N && !(!M && (w !== 16777215 || N));
      ...
      if (M) {
        let ge = T;
        const K = w !== 16777215 && !N, Q = S.width <= y.source.width && S.height <= y.source.height;
        K && Q && (ge = d.getTintedCanvas({ texture: y }, w));
        const me = r.createPattern(ge, ae);
        if (!me) continue;
        const _e = ue, Pe = fe;
        if (_e === 0 || Pe === 0) continue;
        const Me = 1 / _e, be = 1 / Pe, we = (u.x1 - u.x0) * Me, ke = (u.y1 - u.y0) * Me, Se = (u.x3 - u.x0) * be, Ie = (u.y3 - u.y0) * be, Ye = u.x0 - we * oe - Se * re, je = u.y0 - ke * oe - Ie * re, ve = y.source.pixelWidth, ye = y.source.pixelHeight;
        ...
        const K = w !== 16777215 || N ? d.getTintedCanvas({ texture: y }, w) : T, Q = K !== T;
        r.drawImage(
          K,
          Q ? 0 : m,
          Q ? 0 : C,
...
class Ue {
  ...
  execute(e) {
    ...
    for (let p = 0; p < i.length; p++) {
      ...
      for (let U = 0; U < V.length; U++) {
        ...
        const P = b.transform, X = P && !P.isIdentity();
        X && (n.save(), n.transform(P.a, P.b, P.c, P.d, P.tx, P.ty)), De(n, b.shape), h = ut(n, b.holes) || h, c = !0, X && n.restore();
      }
    }
    if (!c) {
  ...
  _warnOnce(e, t) {
    this._warnedMaskTypes.has(e) || (this._warnedMaskTypes.add(e), $e(t));
  }
}
Ue.extension = {
  type: [
  ...
  execute(e, t) {
    ...
    for (let ne = 0; ne < H.length; ne++) {
      ...
      if (J.action === "texture") {
        ...
        let B = G.x * O, W = G.y * O;
        const N = G.width * O, u = G.height * O;
        k !== _ && (B = 0, W = 0);
        const Y = m.transform, j = Y && !Y.isIdentity(), M = C.rotate;
        j || M ? (ie.copyFrom(r), j && ie.append(Y), M && D.matrixAppendRotationInv(ie, M, m.dx, m.dy, m.dw, m.dh), n.setContextTransform(ie, v === 1)) : n.setContextTransform(r, v === 1), s.drawImage(
          k,
          B,
          W,
          k === _ ? N : k.width,
          ...
          m.dw,
          m.dh
        ), (j || M) && n.setContextTransform(r, v === 1);
        continue;
      }
      ...
      const $ = S.shapePrimitives;
      if (!q && ((te = (se = (X = w.hole) == null ? void 0 : X.shapePath) == null ? void 0 : se.shapePrimitives) != null && te.length)) {
        const m = $[$.length - 1];
      ...
      for (let m = 0; m < $.length; m++) {
        ...
        if (L && (s.save(), s.transform(_.a, _.b, _.c, _.d, _.tx, _.ty)), q) {
          ...
          if (W.alignment !== 0.5 && !W.pixelLine) {
            const u = [], Y = [], j = [], M = Je[C.shape.type];
            if (M != null && M.build(C.shape, u)) {
              const Te = C.shape.closePath ?? !0;
              Ke(u, W, !1, Te, Y, j), s.fillStyle = B, Ct(s, Y, j);
            } else
...
he.handleByNamedList(R.CanvasPipesAdaptor, Ne);
he.add(...bt, ...wt, ...kt);
class At extends tt {
  constructor() {
    const e = {
